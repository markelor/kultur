          ...
          if (!req.body.username) {
            ...
            if (!req.body.password) {
              ...
              if (!req.body.privacyPolicy) {
                ...
                if (!req.body.useTerms) {
                  res.json({ success: false, message: eval(language + '.register.useTermsProvidedError') }); // Return error
                } else {
        ...
        User.find({
          ...
          if (err) {
            ...
            var mailOptions = {
              ...
              html: 'The following error has been reported in the kulturekintzak:<br><br>' + err
            };
            // Function to send e-mail to myself
            transporter.sendMail(mailOptions, function(err, info) {
              if (err) {
                      ...
                      if (saveErrorPermission) {
                        ...
                        user.save(function(err) {
                          if (err) {
                            ...
                            res.json({ success: true, message: eval(language + '.editUser.userUpdated') }); // Return success message
                          }
                        });
                      }
                    } else {
